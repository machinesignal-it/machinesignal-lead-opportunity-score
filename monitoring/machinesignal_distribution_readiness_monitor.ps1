...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "mcp_agent_registry_draft_rehearsal_nowrite_probe_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "MCP Agent Registry Draft Rehearsal NoWrite Probe JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Mcp-agent-registry-draft-rehearsal-nowrite-probe-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "mcp_agent_registry_draft_rehearsal_nowrite_probe_summary_20260611.json" },
