...
const OUTPUT_REPORT = "machine_buyer_end_to_end_rehearsal_report_20260611.md";
const OUTPUT_SUMMARY = "machine_buyer_end_to_end_rehearsal_summary_20260611.json";
const MAX_POST_CALLS = 5;
const RUN_ID = `machine-buyer-e2e-${new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14)}`;
...
async function run() {
  ...
  const sandbox = await postJson(
    "/v1/sandbox/customers",
    {
      ...
      expected_test_path: "discover_no_list_score_buy_deep_then_action_pack"
    },
    null,
    `${RUN_ID}-sandbox`
  );
  ...
  const summary = {
    ...
    probe_name: "machine_buyer_end_to_end_rehearsal",
    status: failed.length === 0
      ? "completed_machine_buyer_end_to_end_rehearsal"
      : "failed_machine_buyer_end_to_end_rehearsal",
    ok: failed.length === 0,
    ...
    recommended_next_step: failed.length === 0
      ...
      : "Fix failed end-to-end checks before using the machine-buyer flow as go-live evidence.",
    interpretation: failed.length === 0
      ...
      : "The machine-buyer end-to-end route is not yet clean enough for go-live evidence.",
    resources,
    actions,
    checks
  };

  const report = `# MachineSignal - Machine Buyer End-to-End Rehearsal - 2026-06-11
