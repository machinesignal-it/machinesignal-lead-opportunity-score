...
$resources = @(
    ...
    @{ name = "mcp_tool_manifest"; url = "$PublicSite/mcp-tool-manifest.json"; json = $true; must = "get_external_submission_pack_no_write_review" },
    @{ name = "well_known_mcp_tool_manifest"; url = "$PublicSite/.well-known/mcp-tool-manifest.json"; json = $true; must = "get_external_submission_pack_no_write_review" },
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "external_submission_pack_no_write_review_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "External Submission Pack NoWrite Review JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "External-submission-pack-nowrite-review-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "external_submission_pack_no_write_review_summary_20260608.json" },
