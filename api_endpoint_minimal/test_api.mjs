...
assert.ok(machineOnboardingPayload.callable_flow.some((item) => item.call === "POST /v1/intake"));
...
const intakeNoListResponse = await handleRequest(
  new Request("http://localhost/v1/intake", {
    ...
    body: JSON.stringify({
      ...
      market: "dentisti",
      area: "Milano",
      commercial_objective: "find target domains worth evaluating"
    }),
    headers: {
      "content-type": "application/json",
...
assert.equal(intakeNoListResponse.status, 200);
const intakeNoListPayload = await intakeNoListResponse.json();
assert.equal(intakeNoListPayload.credit_consumed, false);
assert.equal(intakeNoListPayload.recommendation.recommended_product_code, "target_discovery");
assert.equal(intakeNoListPayload.recommendation.billing_code, "MS-BND-DISC-250");
assert.equal(intakeNoListPayload.recommendation.next_api_call.endpoint, "/v1/purchase-intent");

const intakeReliableListResponse = await handleRequest(
  new Request("http://localhost/v1/intake", {
    method: "POST",
    body: JSON.stringify({
      has_list: true,
...
assert.equal(intakeReliableListResponse.status, 200);
const intakeReliableListPayload = await intakeReliableListResponse.json();
assert.equal(intakeReliableListPayload.recommendation.recommended_product_code, "score_pack_1k");
assert.equal(intakeReliableListPayload.recommendation.billing_code, "MS-DEC-250");
assert.equal(intakeReliableListPayload.recommendation.next_api_call.endpoint, "/v1/lead-opportunity-score");

const intakeUnreliableListResponse = await handleRequest(
  new Request("http://localhost/v1/intake", {
    method: "POST",
    body: JSON.stringify({
      has_list: true,
      list_quality: "incomplete_domains",
      batch_id: "customer-list-needs-domain-check",
      area: "Milano"
    }),
    headers: {
      "content-type": "application/json",
...
assert.equal(intakeUnreliableListResponse.status, 200);
const intakeUnreliableListPayload = await intakeUnreliableListResponse.json();
assert.equal(intakeUnreliableListPayload.recommendation.recommended_product_code, "domain_enrichment");
assert.equal(intakeUnreliableListPayload.recommendation.billing_code, "MS-BND-DOM-100");

const intakeUnauthorizedResponse = await handleRequest(
  new Request("http://localhost/v1/intake", {
    method: "POST",
    body: JSON.stringify({ has_list: false }),
    headers: { "content-type": "application/json" }
  }),
  { MACHINESIGNAL_API_KEY: "test-key" }
);
assert.equal(intakeUnauthorizedResponse.status, 401);
