...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "mcp_purchase_decision_probe_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "MCP Purchase Decision Probe JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Mcp-purchase-decision-probe-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "mcp_purchase_decision_probe_summary_20260610.json" },
