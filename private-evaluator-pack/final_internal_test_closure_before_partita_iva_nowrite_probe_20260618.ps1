...
$jsonPath = Join-Path $pack "final_internal_test_closure_before_partita_iva_nowrite_20260618.json"
$mdPath = Join-Path $pack "final_internal_test_closure_before_partita_iva_nowrite_20260618.md"
$summaryPath = Join-Path $pack "final_internal_test_closure_before_partita_iva_nowrite_probe_summary_20260618.json"
$reportPath = Join-Path $pack "final_internal_test_closure_before_partita_iva_nowrite_probe_report_20260618.md"

$sourcePaths = @{
  testPhase = Join-Path $pack "test_phase_completion_gate_nowrite_probe_summary_20260614.json"
  fiscalStop = Join-Path $pack "test_completion_and_partita_iva_stop_gate_nowrite_probe_summary_20260618.json"
  ...
  remainingCoverage = Join-Path $pack "remaining_gate_coverage_review_nowrite_probe_summary_20260618.json"
...
$fiscalStop = Get-Content $sourcePaths.fiscalStop -Raw | ConvertFrom-Json
$fiscal = Get-Content $sourcePaths.fiscal -Raw | ConvertFrom-Json
$payment = Get-Content $sourcePaths.payment -Raw | ConvertFrom-Json
$remainingCoverage = Get-Content $sourcePaths.remainingCoverage -Raw | ConvertFrom-Json
...
Add-Check "source_test_phase_stop_after_tests" ($testPhase.automation_should_stop -eq $true) "automation_should_stop=$($testPhase.automation_should_stop)"
Add-Check "source_test_phase_next_owner_required" ($testPhase.recommended_next_step -eq "owner_decision_required_before_continuing") $testPhase.recommended_next_step
Add-Check "source_fiscal_stop_success" ($fiscalStop.success -eq $true -and $fiscalStop.failed -eq 0) "success=$($fiscalStop.success) failed=$($fiscalStop.failed)"
Add-Check "source_fiscal_stop_no_partita_for_tests" ($fiscalStop.partita_iva_required_now_for_tests -eq $false) "required=$($fiscalStop.partita_iva_required_now_for_tests)"
Add-Check "source_fiscal_stop_before_paid_beta" ($fiscalStop.must_stop_before_paid_beta -eq $true) "stop=$($fiscalStop.must_stop_before_paid_beta)"
Add-Check "source_fiscal_stop_before_payment" ($fiscalStop.must_stop_before_real_payment -eq $true) "stop=$($fiscalStop.must_stop_before_real_payment)"
...
Add-Check "source_remaining_coverage_success" ($remainingCoverage.success -eq $true -and $remainingCoverage.failed -eq 0 -and $remainingCoverage.yellow_gates_with_verified_nowrite_evidence -eq 12) "verified=$($remainingCoverage.yellow_gates_with_verified_nowrite_evidence)"
Add-Check "source_remaining_coverage_no_activation" ($remainingCoverage.activation_allowed -eq $false -and $remainingCoverage.remaining_red_gate -eq "owner_commercial_approval") "red=$($remainingCoverage.remaining_red_gate)"
...
Add-Check "status_exact" ($json.status -eq "final_internal_test_closure_before_partita_iva_ready_nowrite_not_signed_not_activated") $json.status
Add-Check "mode_exact" ($json.mode -eq "final internal test closure and fiscal commercial stop gate only") $json.mode
Add-Check "current_result_not_yet" ($json.current_result -eq "NOT_YET_OWNER_REVIEW_REQUIRED") $json.current_result
Add-Check "decision_exact" ($json.decision -eq "INTERNAL_TESTS_COMPLETE_STOP_BEFORE_FISCAL_COMMERCIAL_TRIGGER") $json.decision
Add-Check "commercial_not_live" ($json.commercial_status -eq "not_live") $json.commercial_status
Add-Check "internal_tests_complete" ($json.internal_tests_complete_for_nowrite_scope -eq $true) "complete=$($json.internal_tests_complete_for_nowrite_scope)"
Add-Check "partita_iva_not_required_for_tests" ($json.partita_iva_required_now_for_tests -eq $false) "required=$($json.partita_iva_required_now_for_tests)"
Add-Check "fiscal_path_not_decided" ($json.fiscal_path_decided -eq $false) "decided=$($json.fiscal_path_decided)"
Add-Check "next_commercial_trigger_stops" ($json.next_commercial_trigger_requires_stop -eq $true) "stop=$($json.next_commercial_trigger_requires_stop)"
Add-Check "allowed_count" ($json.allowed_without_partita_iva.Count -eq 9) "count=$($json.allowed_without_partita_iva.Count)"
Add-Check "blocked_count" ($json.blocked_until_fiscal_path.Count -eq 12) "count=$($json.blocked_until_fiscal_path.Count)"
Add-Check "recommended_next_safe" ($json.recommended_next_safe_action -eq "stop_before_commercial_trigger_or_prepare_only_internal_maintenance") $json.recommended_next_safe_action

$trueFields = @(
  "internal_tests_complete_for_nowrite_scope",
  "next_commercial_trigger_requires_stop",
...
$falseFields = @(
  "partita_iva_required_now_for_tests",
  "fiscal_path_decided",
  "is_approval",
  "is_owner_signature",
...
foreach ($item in $requiredAllowed) {
  Add-Check "allowed_$item" ($json.allowed_without_partita_iva -contains $item) $item
}
...
foreach ($item in $requiredBlocked) {
  Add-Check "blocked_$item" ($json.blocked_until_fiscal_path -contains $item) $item
}

$machine = $json.current_machine_response
Add-Check "machine_status" ($machine.status -eq "final_internal_test_closure_before_partita_iva_ready_nowrite") $machine.status
Add-Check "machine_decision" ($machine.decision -eq "INTERNAL_TESTS_COMPLETE_STOP_BEFORE_FISCAL_COMMERCIAL_TRIGGER") $machine.decision
...
Add-Check "machine_internal_complete" ($machine.internal_tests_complete_for_nowrite_scope -eq $true) "complete=$($machine.internal_tests_complete_for_nowrite_scope)"
...
Add-Check "machine_stop_invoice" ($machine.must_stop_before_invoice -eq $true) "stop=$($machine.must_stop_before_invoice)"
Add-Check "machine_fiscal_path_not_decided" ($machine.fiscal_path_decided -eq $false) "decided=$($machine.fiscal_path_decided)"
Add-Check "machine_next_safe" ($machine.next_safe_action -eq "stop_before_commercial_trigger_or_prepare_only_internal_maintenance") $machine.next_safe_action
Add-Check "machine_support_code" ($machine.support_code -eq "FINAL_INTERNAL_TEST_CLOSURE_BEFORE_PARTITA_IVA_READY_NOWRITE") $machine.support_code
...
$requiredMarkdown = @(
  ...
  "test interni/sandbox risultano completati",
  "INTERNAL_TESTS_COMPLETE_STOP_BEFORE_FISCAL_COMMERCIAL_TRIGGER",
  "partita IVA non e' richiesta ora per i test",
...
$summary = [pscustomobject]@{
  probe = "final_internal_test_closure_before_partita_iva_nowrite_probe_20260618"
  success = $success
  passed = $passed.Count
  failed = $failed.Count
  decision = $json.decision
  internal_tests_complete_for_nowrite_scope = $json.internal_tests_complete_for_nowrite_scope
...
$report = @()
$report += "# Final internal test closure before partita IVA NoWrite probe report"
$report += ""
...
$report += "Internal tests complete for NoWrite scope: $($json.internal_tests_complete_for_nowrite_scope)"
...
if (-not $success) {
  throw "Final internal test closure before partita IVA NoWrite probe failed with $($failed.Count) failed checks."
}
