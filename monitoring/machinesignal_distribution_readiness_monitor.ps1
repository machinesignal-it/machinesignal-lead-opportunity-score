...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "mcp_local_adapter_nowrite_validation_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "MCP Local Adapter NoWrite Validation JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Mcp-local-adapter-nowrite-validation-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "mcp_local_adapter_nowrite_validation_summary_20260610.json" },
