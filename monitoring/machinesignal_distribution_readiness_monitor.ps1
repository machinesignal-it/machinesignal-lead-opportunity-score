...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "external_sandbox_publication_drafts_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "API Directory and RapidAPI Draft Checklist JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "API-directory-RapidAPI-draft-checklist-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "api_directory_rapidapi_draft_checklist_20260607.json" },
