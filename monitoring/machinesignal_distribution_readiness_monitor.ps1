...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "mcp_action_pack_deep_analysis_gate_probe_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "MCP Action Pack Deep Analysis Gate Probe JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Mcp-action-pack-deep-analysis-gate-probe-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "mcp_action_pack_deep_analysis_gate_probe_summary_20260610.json" },
