...
$resources = @(
    ...
    @{ name = "mcp_tool_manifest"; url = "$PublicSite/mcp-tool-manifest.json"; json = $true; must = "get_private_draft_submission_rehearsal" },
    @{ name = "well_known_mcp_tool_manifest"; url = "$PublicSite/.well-known/mcp-tool-manifest.json"; json = $true; must = "get_private_draft_submission_rehearsal" },
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "private_draft_submission_rehearsal_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "Private Draft Submission Rehearsal JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Private-draft-submission-rehearsal-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "private_draft_submission_rehearsal_summary_20260608.json" },
