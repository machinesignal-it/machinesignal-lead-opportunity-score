...
This test simulates a machine customer that does not already have a list:
- create a beta customer;
- buy Target Discovery;
- score all beta sample targets returned by the delivery;
- buy recommended add-ons with prudent Action Pack gating;
- run the admin audit report.
...
OUTPUT_DIR = Path(os.environ.get("MACHINESIGNAL_NO_LIST_OUTPUT_DIR", ".")).resolve()
...
DISCOVERY_REQUEST = {
    ...
    "commercial_objective": (
        "find dental clinic websites worth scoring for digital presence improvement "
        "and machine-prepared commercial action opportunities"
    ),
    "max_budget_eur": UNIT_PRICES_EUR["target_discovery"],
    "reason": "No-list machine flow: customer machine needs a starting batch before score calls.",
}
...
def request_json(
    ...
    headers = {
        ...
        "User-Agent": "MachineSignalNoListDiscoveryFunnel/2026-06-02",
    }
    body = None
...
def run() -> dict[str, Any]:
    ...
    run_id = f"no-list-discovery-{stamp}-{int(time.time())}"
    customer_id = f"no_list_discovery_{stamp.lower()}"
    ...
    status, customer = request_json(
        ...
        payload={
            ...
            "plan": "no_list_target_discovery_funnel_test",
            "customer_type": "beta_customer_no_list_target_discovery_funnel_test",
            "score_credits": 50,
            "deep_analysis_credits": 25,
            ...
            "created_by": "agent_no_list_target_discovery_funnel_test",
        },
    )
    ...
    discovery_ok = (
        ...
        and discovery.get("status") == "accepted_beta_order_intent"
    )
    discovery_order_id = order_id_from(discovery)
    discovery_delivery = discovery.get("delivery") if isinstance(discovery, dict) else {}
    targets = discovery_delivery.get("beta_sample_targets") if isinstance(discovery_delivery, dict) else []
    check("target_discovery_purchased", discovery_ok, f"HTTP {status}, order={discovery_order_id}")
    check("target_discovery_returned_targets", bool(targets), f"targets={len(targets or [])}")
    rows.append({
        "stage": "target_discovery",
        "product_code": "target_discovery",
        "order_intent_id": discovery_order_id,
        "status": discovery.get("status") if isinstance(discovery, dict) else None,
        "target_count_returned": len(targets or []),
    })
    if discovery_ok:
    ...
    for index, target in enumerate(targets or [], start=1):
        ...
        sector_hint = str(target.get("category") or DISCOVERY_REQUEST["market"]).strip()
        area = str(target.get("area") or DISCOVERY_REQUEST["area"]).strip()
        if not domain:
            continue
        ...
        status, score = request_json(
            ...
            payload={
                ...
                "category_hint": sector_hint,
                "initial_signals": target.get("initial_signals"),
                "commercial_objective": DISCOVERY_REQUEST["commercial_objective"],
            },
        )
        ...
        status, purchase = request_json(
            ...
            payload={
                ...
                "reason": f"No-list flow: discovered target score decision {score.get('decision')} recommended {next_product}",
        ...
        add_on_order_id = order_id_from(purchase)
        add_on_row = {
            **score_row,
            "stage": "purchase",
            "product_code": next_product,
            "order_intent_id": add_on_order_id,
        ...
        if should_buy_action:
            ...
            rows.append({
                **score_row,
                ...
                "source_deep_order_intent_id": add_on_order_id,
            })
            continue
        ...
        status, action_purchase = request_json(
            ...
            payload={
                ...
                "domain": domain,
                "source_score_request_id": score.get("request_id"),
                "source_order_intent_id": add_on_order_id,
                "reason": f"No-list flow natural gate passed after deep analysis: {action_reason}",
    ...
    downstream_revenue = float(audit_summary.get("simulated_revenue_eur") or 0) - target_discovery_revenue
    score_count = len(score_rows)

    return {
        "ok": not [item for item in checks if not item["ok"]],
        "test_name": "no_list_target_discovery_funnel_test",
        "finished_at": datetime.now().isoformat(timespec="seconds"),
        "customer_id": customer_id,
        "checks": checks,
        "summary": {
            "target_discovery_order_id": discovery_order_id,
            "targets_returned": len(targets or []),
            "scores_completed": score_count,
            "score_failures": score_failures,
            "purchase_failures": purchase_failures,
            "orders_count": audit_summary.get("order_count"),
            "simulated_revenue_eur": audit_summary.get("simulated_revenue_eur"),
            "target_discovery_revenue_eur": round(target_discovery_revenue, 2),
            "downstream_revenue_eur": round(downstream_revenue, 2),
            "downstream_revenue_per_scored_target_eur": round(downstream_revenue / score_count, 4) if score_count else 0,
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Esito sintetico",
        "",
        f"- Target Discovery order: `{s['target_discovery_order_id']}`",
        f"- Target beta restituiti: {s['targets_returned']}",
        ...
        f"- Ricavo downstream dopo discovery: {s['downstream_revenue_eur']} EUR",
        f"- Ricavo downstream per target segnato: {s['downstream_revenue_per_scored_target_eur']} EUR",
        "",
        "## Mix",
        "",
    ...
    for row in result.get("rows", []):
        lines.append(
            f"| {row.get('stage')} | {row.get('domain', '')} | {row.get('score', '')} | "
            f"{row.get('confidence', '')} | {row.get('decision', '')} | "
            f"{row.get('product_code') or row.get('next_product') or ''} | "
            f"{row.get('action_pack_gate_reason') or row.get('status') or ''} |"
        )
...
def main() -> int:
    ...
    json_path = OUTPUT_DIR / f"no_list_target_discovery_funnel_summary_{suffix}.json"
    md_path = OUTPUT_DIR / f"no_list_target_discovery_funnel_report_{suffix}.md"
    csv_path = OUTPUT_DIR / f"no_list_target_discovery_funnel_rows_{suffix}.csv"
