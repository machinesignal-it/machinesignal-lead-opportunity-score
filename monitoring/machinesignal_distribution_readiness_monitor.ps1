...
$resources = @(
    ...
    @{ name = "well_known_machine_discovery"; url = "$PublicSite/.well-known/machine-discovery.json"; json = $true; must = "mcp_deep_analysis_verification_gate_probe_json" },
    @{ name = "llms"; url = "$PublicSite/llms.txt"; json = $false; must = "MCP Deep Analysis Verification Gate Probe JSON" },
    @{ name = "robots"; url = "$PublicSite/robots.txt"; json = $false; must = "Mcp-deep-analysis-verification-gate-probe-json" },
    @{ name = "sitemap"; url = "$PublicSite/sitemap.xml"; json = $false; must = "mcp_deep_analysis_verification_gate_probe_summary_20260610.json" },
